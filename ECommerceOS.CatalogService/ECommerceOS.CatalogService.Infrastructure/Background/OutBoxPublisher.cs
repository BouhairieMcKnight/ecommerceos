...
namespace ECommerceOS.CatalogService.Infrastructure.Background;

public class OutBoxPublisher(
    ILogger<OutBoxPublisher> logger,
    NpgsqlDataSource dataSource,
    IProducer<string, CatalogEvent> producer,
    ...
    : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var publishTasks = messages.Select(async outbox =>
        {
            try
            {
                var identityEvent = new CatalogEvent
                {
                ...
                var message = new Message<string, CatalogEvent>
                {
                    Key = "catalog-event",
                    Value = identityEvent
                };
