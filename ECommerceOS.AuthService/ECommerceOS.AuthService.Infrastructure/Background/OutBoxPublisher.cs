...
public class OutBoxPublisher(
    ...
    IProducer<string, IdentityEvent> producer,
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var publishTasks = messages.Select(async outbox =>
        {
            try
            {
                ...
                var message = new Message<string, IdentityEvent>
                {
                    Key = "identity-event",
                    Value = identityEvent
                };
                
                await producer.ProduceAsync("identity-event", message, context.CancellationToken);
